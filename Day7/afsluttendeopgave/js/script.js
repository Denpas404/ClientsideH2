...
const swapiApi = (async function () {
  ...
  async function ShowCard(data) {
    ignorIsSet = false;
    cardContainer.className = "singleCardContainer";
    btnContainer.innerHTML = "";
    cardContainer.innerHTML = "";

    let card = document.createElement("div");
    card.className = "singleCard";
    card.innerHTML = (await generateCardContent(data)).toString();
  ...
  async function generateCardContent(dataItem) {
    ...
    for (let [key, value] of Object.entries(dataItem)) {
      ...
      if (ignor.includes(key) && ignorIsSet) {
        continue;
      }
      if (key == "homeworld" && value != null) {
        ...
        for (let item of value) {
          content += `<span class="value">${item}</span><br>`;
        }
        content += `<hr>`;
      }
